...
public class NPCInitializer : MonoBehaviour
{
    ...
    IEnumerator StaggerNPCSpawns()
    {
        ...
        for (int i = 0; i < numberToSpawn; i ++)
        {
            ...
            Vector3 pos = new Vector3(Random.value * gridSize.x - (gridSize.x / 2), 0, Random.value * gridSize.y - (gridSize.y / 2));
            while (!(grid.NodeFromWorldPoint(pos).walkable))
            {
                pos = new Vector3(Random.value * gridSize.x - (gridSize.x / 2), 0, Random.value * gridSize.y - (gridSize.y / 2));
            }
